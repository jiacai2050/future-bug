...
use futures::executor::{self, Executor as OldExecutor, Run};
use futures::future::{lazy, ExecuteError, Executor};
...
impl<T: Send + std::fmt::Debug + 'static, E: Send + std::fmt::Debug + 'static> Future
    for CpuFuture<T, E>
{
    type Item = T;
    type Error = E;
    ...
    fn poll(&mut self) -> Poll<T, E> {
        match self
            ...
            .expect("cannot poll CpuFuture twice")
        {
...
impl Builder {
    ...
    pub fn create(&mut self) -> CpuPool {
        ...
        for counter in 0..self.pool_size {
            ...
            thread_builder
                .spawn(move || inner.work(after_start, before_stop))
                .unwrap();
        }
        return pool;
    }
...
mod tests {
    ...
    fn test_drop_after_start() {
        ...
        let _cpu_pool = Builder::new()
            .pool_size(2)
            .after_start(move || tx.send(1).unwrap())
            .create();
